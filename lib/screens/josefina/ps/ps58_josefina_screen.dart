...
class PS58JosefinaScreen extends StatefulWidget {
  const PS58JosefinaScreen({super.key});

  @override
  State<PS58JosefinaScreen> createState() => _PS58JosefinaScreenState();
}

class _PS58JosefinaScreenState extends State<PS58JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS58();
          debugPrint('[PS58] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS58');
        debugPrint('[PS58] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS58();
        debugPrint('[PS58] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS58');
      debugPrint('[PS58] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS58?.cancel();
    PiscinasJosefinaProvider.subscriptionPS58 = null;
    super.dispose();
  }
            ...
            SafeArea(
              child: Consumer<PiscinasJosefinaProvider>(
                builder: (context, ebProvider, child) {
                  apiDataList = ebProvider.apiDataListPS58;
                  ...
                  if (ebProvider.hasError) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () => ebProvider.subscribePS58(),
                      ),
                    );
                  }
                  ...
                  if (!datosCompletos) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () {
                          debugPrint('[PS58] Reintentando suscripción...');
                          ebProvider.subscribePS58();
                        },
                      ),
                    );
                  }

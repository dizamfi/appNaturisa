...
class PC117JosefinaScreen extends StatefulWidget {
  const PC117JosefinaScreen({super.key});

  @override
  State<PC117JosefinaScreen> createState() => _PC117JosefinaScreenState();
}

class _PC117JosefinaScreenState extends State<PC117JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePC117();
          debugPrint('[PC117] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPC117');
        debugPrint('[PC117] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePC117();
        debugPrint('[PC117] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPC117');
      debugPrint('[PC117] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPC117?.cancel();
    PiscinasJosefinaProvider.subscriptionPC117 = null;
    super.dispose();
  }
              ...
              SafeArea(
                child: Consumer<PiscinasJosefinaProvider>(
                  builder: (context, ebProvider, child) {
                    apiDataList = ebProvider.apiDataListPC117;
                    ...
                    if (ebProvider.hasError) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () => ebProvider.subscribePC117(),
                        ),
                      );
                    }
                    ...
                    if (!datosCompletos) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () {
                            debugPrint('[PC117] Reintentando suscripción...');
                            ebProvider.subscribePC117();
                          },
                        ),
                      );
                    }
                                      ...
                                      slides: [
                                        CardTableroAire(
                                          ...
                                          statComm:
                                              DataAccessUtils.getNestedValue(
                                                  apiDataList,
                                                  [1, 0, "TA_117_01.Stat_Comm"],
                                                  false),
                                          imputsLN:
                                              DataAccessUtils.getNestedValue(
                                                  apiDataList,
                                                  [1, 0, "TA_117_01.Inputs"],
                                                  0),
                                        ),
                                      ],
                                    ),

...
class PS50JosefinaScreen extends StatefulWidget {
  const PS50JosefinaScreen({super.key});

  @override
  State<PS50JosefinaScreen> createState() => _PS50JosefinaScreenState();
}

class _PS50JosefinaScreenState extends State<PS50JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS50();
          debugPrint('[PS50] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS50');
        debugPrint('[PS50] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS50();
        debugPrint('[PS50] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS50');
      debugPrint('[PS50] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS50?.cancel();
    PiscinasJosefinaProvider.subscriptionPS50 = null;
    super.dispose();
  }
            ...
            SafeArea(
              child: Consumer<PiscinasJosefinaProvider>(
                builder: (context, ebProvider, child) {
                  apiDataList = ebProvider.apiDataListPS50;
                  ...
                  if (ebProvider.hasError) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () => ebProvider.subscribePS50(),
                      ),
                    );
                  }
                  ...
                  if (!datosCompletos) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () {
                          debugPrint('[PS50] Reintentando suscripción...');
                          ebProvider.subscribePS50();
                        },
                      ),
                    );
                  }

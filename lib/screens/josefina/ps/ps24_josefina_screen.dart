...
class PS24JosefinaScreen extends StatefulWidget {
  const PS24JosefinaScreen({super.key});

  @override
  State<PS24JosefinaScreen> createState() => _PS24JosefinaScreenState();
}

class _PS24JosefinaScreenState extends State<PS24JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS24();
          debugPrint('[PS24] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS24');
        debugPrint('[PS24] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS24();
        debugPrint('[PS24] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS24');
      debugPrint('[PS24] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS24?.cancel();
    PiscinasJosefinaProvider.subscriptionPS24 = null;
    super.dispose();
  }
              ...
              SafeArea(
                child: Consumer<PiscinasJosefinaProvider>(
                  builder: (context, ebProvider, child) {
                    apiDataList = ebProvider.apiDataListPS24;
                    ...
                    if (ebProvider.hasError) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () => ebProvider.subscribePS24(),
                        ),
                      );
                    }
                    ...
                    bool datosCompletos = apiDataList.length >= 3 &&
                        ...
                        (apiDataList[1].length >=
                            2) && // Asegurarse de que hay datos para ambos tableros
                    ...
                    if (!datosCompletos) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () {
                            debugPrint('[PS24] Reintentando suscripción...');
                            ebProvider.subscribePS24();
                          },
                        ),
                      );
                    }

...
class PS19JosefinaScreen extends StatefulWidget {
  const PS19JosefinaScreen({super.key});

  @override
  State<PS19JosefinaScreen> createState() => _PS19JosefinaScreenState();
}

class _PS19JosefinaScreenState extends State<PS19JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS19();
          debugPrint('[PS19] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS19');
        debugPrint('[PS19] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS19();
        debugPrint('[PS19] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS19');
      debugPrint('[PS19] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS19?.cancel();
    PiscinasJosefinaProvider.subscriptionPS19 = null;
    super.dispose();
  }
              ...
              SafeArea(
                child: Consumer<PiscinasJosefinaProvider>(
                  builder: (context, ebProvider, child) {
                    apiDataList = ebProvider.apiDataListPS19;
                    ...
                    if (ebProvider.hasError) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () => ebProvider.subscribePS19(),
                        ),
                      );
                    }
                    ...
                    if (!datosCompletos) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () {
                            debugPrint('[PS19] Reintentando suscripción...');
                            ebProvider.subscribePS19();
                          },
                        ),
                      );
                    }

...
class PS06JosefinaScreen extends StatefulWidget {
  const PS06JosefinaScreen({super.key});

  @override
  State<PS06JosefinaScreen> createState() => _PS06JosefinaScreenState();
}

class _PS06JosefinaScreenState extends State<PS06JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS06();
          debugPrint('[PS06] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS06');
        debugPrint('[PS06] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS06();
        debugPrint('[PS06] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS06');
      debugPrint('[PS06] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS06?.cancel();
    PiscinasJosefinaProvider.subscriptionPS06 = null;
    super.dispose();
  }
              ...
              SafeArea(
                child: Consumer<PiscinasJosefinaProvider>(
                  builder: (context, ebProvider, child) {
                    apiDataList = ebProvider.apiDataListPS06;
                    ...
                    if (ebProvider.hasError) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () => ebProvider.subscribePS06(),
                        ),
                      );
                    }
                    ...
                    if (!datosCompletos) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () {
                            debugPrint('[PS06] Reintentando suscripción...');
                            ebProvider.subscribePS06();
                          },
                        ),
                      );
                    }

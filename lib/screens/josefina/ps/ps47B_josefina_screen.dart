...
class PS47BJosefinaScreen extends StatefulWidget {
  const PS47BJosefinaScreen({super.key});

  @override
  State<PS47BJosefinaScreen> createState() => _PS47BJosefinaScreenState();
}

class _PS47BJosefinaScreenState extends State<PS47BJosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS47B();
          debugPrint('[PS47B] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS47B');
        debugPrint('[PS47B] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS47B();
        debugPrint('[PS47B] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS47B-2');
      debugPrint('[PS47B] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS47B?.cancel();
    PiscinasJosefinaProvider.subscriptionPS47B = null;
    super.dispose();
  }
            ...
            SafeArea(
              child: Consumer<PiscinasJosefinaProvider>(
                builder: (context, ebProvider, child) {
                  apiDataList = ebProvider.apiDataListPS47B;
                  ...
                  if (ebProvider.hasError) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () => ebProvider.subscribePS47B(),
                      ),
                    );
                  }
                  ...
                  if (!datosCompletos) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () {
                          debugPrint('[PS47B] Reintentando suscripción...');
                          ebProvider.subscribePS47B();
                        },
                      ),
                    );
                  }
                                    ...
                                    slides: [
                                      CardTableroAire(
                                        ...
                                        statComm:
                                            DataAccessUtils.getNestedValue(
                                                apiDataList,
                                                [1, 0, "TA_47B_01.Stat_Comm"],
                                                false),
                                        imputsLN:
                                            DataAccessUtils.getNestedValue(
                                                apiDataList,
                                                [1, 0, "TA_47B_01.Inputs"],
                                                0),
                                      ),
                                    ],
                                  ),

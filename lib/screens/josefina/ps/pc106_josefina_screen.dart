...
class PC106JosefinaScreen extends StatefulWidget {
  const PC106JosefinaScreen({super.key});

  @override
  State<PC106JosefinaScreen> createState() => _PC106JosefinaScreenState();
}

class _PC106JosefinaScreenState extends State<PC106JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePC106();
          debugPrint('[PC106] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPC106');
        debugPrint('[PC106] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePC106();
        debugPrint('[PC106] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPC106');
      debugPrint('[PC106] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPC106?.cancel();
    PiscinasJosefinaProvider.subscriptionPC106 = null;
    super.dispose();
  }
              ...
              SafeArea(
                child: Consumer<PiscinasJosefinaProvider>(
                  builder: (context, ebProvider, child) {
                    apiDataList = ebProvider.apiDataListPC106;
                    ...
                    if (ebProvider.hasError) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () => ebProvider.subscribePC106(),
                        ),
                      );
                    }
                    ...
                    if (!datosCompletos) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () {
                            debugPrint('[PC106] Reintentando suscripción...');
                            ebProvider.subscribePC106();
                          },
                        ),
                      );
                    }
                                      ...
                                      slides: [
                                        CardTableroAire(
                                          ...
                                          statComm:
                                              DataAccessUtils.getNestedValue(
                                                  apiDataList,
                                                  [1, 0, "TA_106_01.Stat_Comm"],
                                                  false),
                                          imputsLN:
                                              DataAccessUtils.getNestedValue(
                                                  apiDataList,
                                                  [1, 0, "TA_106_01.Inputs"],
                                                  0),
                                        ),
                                      ],
                                    ),
                                  ...
                                  Container(
                                    ...
                                    child: SlideShow(
                                      ...
                                      colorPrimario:
                                          const Color.fromARGB(255, 4, 63, 122),
                                      slides: [
                                        // Los widgets ControlRemoto y InfoGeneralPS ya están usando DataAccessUtils
                                        // internamente para manejar datos incompletos, así que no necesitamos

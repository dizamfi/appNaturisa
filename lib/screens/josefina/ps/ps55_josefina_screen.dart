...
class PS55JosefinaScreen extends StatefulWidget {
  const PS55JosefinaScreen({super.key});

  @override
  State<PS55JosefinaScreen> createState() => _PS55JosefinaScreenState();
}

class _PS55JosefinaScreenState extends State<PS55JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS55();
          debugPrint('[PS55] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS55');
        debugPrint('[PS55] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS55();
        debugPrint('[PS55] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS55');
      debugPrint('[PS55] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS55?.cancel();
    PiscinasJosefinaProvider.subscriptionPS55 = null;
    super.dispose();
  }
            ...
            SafeArea(
              child: Consumer<PiscinasJosefinaProvider>(
                builder: (context, ebProvider, child) {
                  apiDataList = ebProvider.apiDataListPS55;
                  ...
                  if (ebProvider.hasError) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () => ebProvider.subscribePS55(),
                      ),
                    );
                  }
                  ...
                  if (!datosCompletos) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () {
                          debugPrint('[PS55] Reintentando suscripción...');
                          ebProvider.subscribePS55();
                        },
                      ),
                    );
                  }

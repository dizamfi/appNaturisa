...
class PC108JosefinaScreen extends StatefulWidget {
  const PC108JosefinaScreen({super.key});

  @override
  State<PC108JosefinaScreen> createState() => _PC108JosefinaScreenState();
}

class _PC108JosefinaScreenState extends State<PC108JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePC108();
          debugPrint('[PC108] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPC108');
        debugPrint('[PC108] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePC108();
        debugPrint('[PC108] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPC108');
      debugPrint('[PC108] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPC108?.cancel();
    PiscinasJosefinaProvider.subscriptionPC108 = null;
    super.dispose();
  }
              ...
              SafeArea(
                child: Consumer<PiscinasJosefinaProvider>(
                  builder: (context, ebProvider, child) {
                    apiDataList = ebProvider.apiDataListPC108;
                    ...
                    if (ebProvider.hasError) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () => ebProvider.subscribePC108(),
                        ),
                      );
                    }
                    ...
                    if (!datosCompletos) {
                      return Center(
                        child: ErrorScreen(
                          ...
                          onRetry: () {
                            debugPrint('[PC108] Reintentando suscripción...');
                            ebProvider.subscribePC108();
                          },
                        ),
                      );
                    }
                                      ...
                                      slides: [
                                        CardTableroAire(
                                          ...
                                          statComm:
                                              DataAccessUtils.getNestedValue(
                                                  apiDataList,
                                                  [1, 0, "TA_108_01.Stat_Comm"],
                                                  false),
                                          imputsLN:
                                              DataAccessUtils.getNestedValue(
                                                  apiDataList,
                                                  [1, 0, "TA_108_01.Inputs"],
                                                  0),
                                        ),
                                      ],
                                    ),

...
class PS57JosefinaScreen extends StatefulWidget {
  const PS57JosefinaScreen({super.key});

  @override
  State<PS57JosefinaScreen> createState() => _PS57JosefinaScreenState();
}

class _PS57JosefinaScreenState extends State<PS57JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS57();
          debugPrint('[PS57] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS57');
        debugPrint('[PS57] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS57();
        debugPrint('[PS57] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS57');
      debugPrint('[PS57] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS57?.cancel();
    PiscinasJosefinaProvider.subscriptionPS57 = null;
    super.dispose();
  }
            ...
            SafeArea(
              child: Consumer<PiscinasJosefinaProvider>(
                builder: (context, ebProvider, child) {
                  apiDataList = ebProvider.apiDataListPS57;
                  ...
                  if (ebProvider.hasError) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () => ebProvider.subscribePS57(),
                      ),
                    );
                  }
                  ...
                  if (!datosCompletos) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () {
                          debugPrint('[PS57] Reintentando suscripción...');
                          ebProvider.subscribePS57();
                        },
                      ),
                    );
                  }

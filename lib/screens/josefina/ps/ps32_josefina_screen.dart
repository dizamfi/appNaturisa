...
class PS32JosefinaScreen extends StatefulWidget {
  const PS32JosefinaScreen({super.key});

  @override
  State<PS32JosefinaScreen> createState() => _PS32JosefinaScreenState();
}

class _PS32JosefinaScreenState extends State<PS32JosefinaScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (firstLoad) {
        if (PiscinasJosefinaProvider.channel != null) {
          Provider.of<PiscinasJosefinaProvider>(context, listen: false)
              .subscribePS32();
          debugPrint('[PS32] Suscripción inicial realizada');
        }
        setState(() {
          firstLoad = false;
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (PiscinasJosefinaProvider.channel != null) {
        PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS32');
        debugPrint('[PS32] Cancelando suscripción (app en segundo plano)');
      }
    } else if (state == AppLifecycleState.resumed) {
      if (PiscinasJosefinaProvider.channel != null) {
        Provider.of<PiscinasJosefinaProvider>(context, listen: false)
            .subscribePS32();
        debugPrint('[PS32] Resuscripción al volver a primer plano');
      }
    }
  }
  ...
  void dispose() {
    ...
    if (PiscinasJosefinaProvider.channel != null) {
      PiscinasJosefinaProvider.channel!.sink.add('unsubscribeFJOSPS32');
      debugPrint('[PS32] Cancelando suscripción (dispose)');
    }
    PiscinasJosefinaProvider.subscriptionPS32?.cancel();
    PiscinasJosefinaProvider.subscriptionPS32 = null;
    super.dispose();
  }
            ...
            SafeArea(
              child: Consumer<PiscinasJosefinaProvider>(
                builder: (context, ebProvider, child) {
                  apiDataList = ebProvider.apiDataListPS32;
                  ...
                  if (ebProvider.hasError) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () => ebProvider.subscribePS32(),
                      ),
                    );
                  }
                  ...
                  if (!datosCompletos) {
                    return Center(
                      child: ErrorScreen(
                        ...
                        onRetry: () {
                          debugPrint('[PS32] Reintentando suscripción...');
                          ebProvider.subscribePS32();
                        },
                      ),
                    );
                  }
